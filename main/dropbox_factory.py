...
def dbx_init_token(photo_bytes, message, bot, values):
...
def after_init_token(message, bot, auth_flow, photo_bytes, values):
    ...
    bot.register_next_step_handler(message, upload_file, photo_bytes, message, bot, values)


def get_dbx(photo_bytes, message, bot, values):
    ...
    try:
        ...
        dbx_init_token(photo_bytes, message, bot, values)


def upload_file(photo_bytes, message, bot, values):
    dbx = get_dbx(photo_bytes, message, bot, values)
    try:
        data = pandas.read_sql('consumer', engine)
        path = "/FlowImages/" + values.get('type') + "/" + values.get('type') + str(max(
